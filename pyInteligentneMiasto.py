...
class pyIntelligentCity(object):
    ...
    def arduino_detect(self):
        try:
            ...
            self.IsCity = True
            print('Done OK - connecting Arduino with IntelligentCity at COM Port: ' + ComPort)
            return True
        except:
            self.IsCity = False
    ...
    def device_detect(self):
        ...
        for port in range(4):
            ...
            if a <=1010 and a >=990: # sygnalizator drogowy
                ...
                device_type.add(2)
            elif a <=600 and a>=100:   # lampa uliczna (może być też pusty port!)
                device_type.add(LAMPA)
